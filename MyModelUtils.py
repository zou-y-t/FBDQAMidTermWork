...
class Model1:
    ...
    def fit(self):
        ...
        # 计算R2
        self.R2 = 1-(self.r8 - self.predictR8).dot(self.r8 - self.predictR8) / np.sum((self.r8 - np.mean(self.r8)) ** 2)
...
class Model2:
    ...
    def fit(self):
        ...
        # 计算截距alpha
        self.alpha=np.mean(self.r8)-self.beta_r1*np.mean(self.r1)-self.beta_r7*np.mean(self.r7)
        # 计算预测值
...
class DataCleaner:
    ...
    def __init__(self, df):
        ...
        self.r = [self.r1, self.r2, self.r3, self.r4, self.r5, self.r6, self.r7, self.r8]


    def clean(self):
        # 去除半小时结点的数据
        self.df['date']=pd.to_datetime(self.df['date'])
        self.df = self.df[self.df['date'].dt.time.astype(str).isin(self.half_hour_time)].copy()
        # 计算r
